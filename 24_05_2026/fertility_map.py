...
for year in range(1950, 2031):    
        print(year)
        query = """     
                        SELECT   D.[iso3_code] AS [country_code] ,D.[value]
                        ...
                        WHERE D.[year] = """ + str(year) + """ AND ENT.[is_country] = 1 and [indicator_id] = 19 AND D.[variant_id] = 4
                """

        MAP_TITLE = 'Total fertility rate'
        MAP_SUBTITLE = str(year)
        MAP_SOURCE = 'Source: United Nations'
        FILE_NAME = 'fertility_'+ str(year) + '.png'
        LEGEND_MIN = 0
        LEGEND_MAX = 9
