...
"""The world's largest urban settlements are increasingly concentrated in Asia.

Number of urban settlements with more than 500,000 inhabitants by country, 1950–2035.

Source: United Nations.

#Urbanization #Demography #DataViz
"""

TOP_N = 10
FILE_NAME = "un_cities_over_500k_1950_2035.mp4"
TITLE = "Countries with the Most Cities Over 500,000 People"
SUBTITLE = "1950–2035 • Historical data and UN projections"
FOOTER_LEFT = "Source: United Nations, World Urbanization Prospects"
...
SELECT 
        CASE 
            ...
            WHEN [location_name] = 'Democratic Republic of the Congo' THEN 'Congo'
            WHEN [location_name] = 'Iran (Islamic Republic of)' THEN 'Iran'
            WHEN [location_name] = 'United Kingdom' THEN 'UK'
            WHEN [location_name] = 'South Africa' THEN 'SA'
            ELSE [location_name]
...
FROM (
    ...
    FROM (
        ...
        WHERE   indicator_id = 105
                AND [category_id] IN (112,113,114,115,116,117)
                AND ENT.is_country = 1) AS A
    GROUP BY [location_name],[iso2_code],[iso3_code],[year]) AS AA
