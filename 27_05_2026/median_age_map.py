...
for year in range(1950, 2036):    
        print(year)
        query = """     
                SELECT   D.[iso3_code] AS [country_code] ,ROUND(D.[value], 1) AS [value]
                ...
                WHERE D.[year] = """ + str(year) + """ AND ENT.[is_country] = 1 and [indicator_id] = 67 AND D.[variant_id] = 4 AND [sex_id] = 3
                """

        MAP_TITLE = 'Median age of population'
        MAP_SUBTITLE = str(year)
        MAP_SOURCE = 'Source: United Nations'
        FILE_NAME = 'median_age__'+ str(year) + '.png'
        LEGEND_MIN = 12
        LEGEND_MAX = 55
        COLOR_SCALE_LOW="#D0FF00"
        COLOR_SCALE_HIGH="#3300FF"
